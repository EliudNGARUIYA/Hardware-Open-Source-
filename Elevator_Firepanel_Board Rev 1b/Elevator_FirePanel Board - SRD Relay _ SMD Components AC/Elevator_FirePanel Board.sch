EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title "Elevator - Fire Panel Board"
Date "2019-06-02"
Rev "1b"
Comp "Aurora Marlin "
Comment1 ""
Comment2 ""
Comment3 "Checked By : Reuben G"
Comment4 "Designed By : Eliud NGARUIYA"
$EndDescr
$Comp
L Connector:Conn_01x03_Male J1
U 1 1 5CF42365
P 6600 2550
F 0 "J1" H 6708 2831 50  0000 C CNN
F 1 "Output" H 6708 2740 50  0000 C CNN
F 2 "TerminalBlock_Phoenix:TerminalBlock_Phoenix_MKDS-1,5-3_1x03_P5.00mm_Horizontal" H 6600 2550 50  0001 C CNN
F 3 "~" H 6600 2550 50  0001 C CNN
	1    6600 2550
	-1   0    0    -1  
$EndComp
$Comp
L Device:D D1
U 1 1 5CF472F1
P 5450 1550
F 0 "D1" H 5450 1334 50  0000 C CNN
F 1 "D" H 5450 1425 50  0000 C CNN
F 2 "Diode_SMD:D_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 5450 1550 50  0001 C CNN
F 3 "~" H 5450 1550 50  0001 C CNN
	1    5450 1550
	-1   0    0    1   
$EndComp
$Comp
L Device:R_Small_US R1
U 1 1 5CF49B4E
P 3850 2400
F 0 "R1" V 3645 2400 50  0000 C CNN
F 1 "10K" V 3736 2400 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 3850 2400 50  0001 C CNN
F 3 "~" H 3850 2400 50  0001 C CNN
	1    3850 2400
	0    1    1    0   
$EndComp
Wire Wire Line
	4050 2400 3950 2400
Wire Wire Line
	3750 2400 3650 2400
Wire Wire Line
	4350 2200 4350 2150
Wire Wire Line
	4350 2150 5050 2150
$Comp
L power:+5V #PWR06
U 1 1 5CF4A9D7
P 5900 1550
F 0 "#PWR06" H 5900 1400 50  0001 C CNN
F 1 "+5V" H 5915 1723 50  0000 C CNN
F 2 "" H 5900 1550 50  0001 C CNN
F 3 "" H 5900 1550 50  0001 C CNN
	1    5900 1550
	1    0    0    -1  
$EndComp
Wire Wire Line
	5750 2150 5800 2150
Wire Wire Line
	5600 1550 5800 1550
Wire Wire Line
	5800 1550 5800 2150
Connection ~ 5800 2150
Wire Wire Line
	5800 2150 5900 2150
Wire Wire Line
	5300 1550 5050 1550
Wire Wire Line
	5050 1550 5050 2150
Connection ~ 5050 2150
Wire Wire Line
	5050 2150 5150 2150
Wire Wire Line
	5900 1550 5900 2150
Wire Wire Line
	5150 2550 4950 2550
Text Label 3650 2400 2    50   ~ 0
Relay
Wire Wire Line
	6400 2550 6100 2550
$Comp
L power:GND #PWR010
U 1 1 5CF63546
P 4300 4550
F 0 "#PWR010" H 4300 4300 50  0001 C CNN
F 1 "GND" H 4305 4377 50  0000 C CNN
F 2 "" H 4300 4550 50  0001 C CNN
F 3 "" H 4300 4550 50  0001 C CNN
	1    4300 4550
	1    0    0    -1  
$EndComp
Wire Wire Line
	4300 4550 4300 4300
$Comp
L Device:LED_Small D2
U 1 1 5CF9ECF9
P 6150 5850
F 0 "D2" V 6196 5782 50  0000 R CNN
F 1 "Green" V 6105 5782 50  0000 R CNN
F 2 "LED_SMD:LED_2010_5025Metric_Castellated" V 6150 5850 50  0001 C CNN
F 3 "~" V 6150 5850 50  0001 C CNN
	1    6150 5850
	0    -1   -1   0   
$EndComp
$Comp
L Device:R_Small_US R6
U 1 1 5CFA0037
P 6150 5550
F 0 "R6" H 6082 5504 50  0000 R CNN
F 1 "330" H 6082 5595 50  0000 R CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 6150 5550 50  0001 C CNN
F 3 "~" H 6150 5550 50  0001 C CNN
	1    6150 5550
	-1   0    0    1   
$EndComp
Wire Wire Line
	6150 5400 6150 5450
Wire Wire Line
	6150 5650 6150 5750
$Comp
L power:GND #PWR017
U 1 1 5CFA2EF1
P 6150 6050
F 0 "#PWR017" H 6150 5800 50  0001 C CNN
F 1 "GND" H 6155 5877 50  0000 C CNN
F 2 "" H 6150 6050 50  0001 C CNN
F 3 "" H 6150 6050 50  0001 C CNN
	1    6150 6050
	1    0    0    -1  
$EndComp
Wire Wire Line
	6150 6050 6150 5950
$Comp
L Connector:Conn_01x02_Male J2
U 1 1 5CF48D9A
P 3950 4200
F 0 "J2" H 4058 4381 50  0000 C CNN
F 1 "Input" H 4058 4290 50  0000 C CNN
F 2 "TerminalBlock_Phoenix:TerminalBlock_Phoenix_MKDS-1,5-2_1x02_P5.00mm_Horizontal" H 3950 4200 50  0001 C CNN
F 3 "~" H 3950 4200 50  0001 C CNN
	1    3950 4200
	1    0    0    -1  
$EndComp
Wire Wire Line
	4150 4300 4300 4300
$Comp
L Transistor_BJT:BC547 Q1
U 1 1 5CF5E51A
P 4250 2400
F 0 "Q1" H 4441 2446 50  0000 L CNN
F 1 "BC547" H 4441 2355 50  0000 L CNN
F 2 "Package_TO_SOT_THT:TO-92_Inline" H 4450 2325 50  0001 L CIN
F 3 "http://www.fairchildsemi.com/ds/BC/BC547.pdf" H 4250 2400 50  0001 L CNN
	1    4250 2400
	1    0    0    -1  
$EndComp
$Comp
L Device:R_US R8
U 1 1 5D015A01
P 10450 5500
AR Path="/5D015A01" Ref="R8"  Part="1" 
AR Path="/5D015685/5D015A01" Ref="R?"  Part="1" 
F 0 "R8" V 10245 5500 50  0000 C CNN
F 1 "330" V 10336 5500 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" V 10490 5490 50  0001 C CNN
F 3 "~" H 10450 5500 50  0001 C CNN
	1    10450 5500
	0    1    1    0   
$EndComp
$Comp
L Switch:SW_Push SW1
U 1 1 5D015A16
P 10400 4250
AR Path="/5D015A16" Ref="SW1"  Part="1" 
AR Path="/5D015685/5D015A16" Ref="SW?"  Part="1" 
F 0 "SW1" V 10354 4398 50  0000 L CNN
F 1 "SW_Push" V 10445 4398 50  0000 L CNN
F 2 "Button_Switch_SMD:SW_SPST_B3U-1000P" H 10400 4450 50  0001 C CNN
F 3 "" H 10400 4450 50  0001 C CNN
	1    10400 4250
	0    1    1    0   
$EndComp
$Comp
L Device:LED D3
U 1 1 5D015A1D
P 10700 5700
AR Path="/5D015A1D" Ref="D3"  Part="1" 
AR Path="/5D015685/5D015A1D" Ref="D?"  Part="1" 
F 0 "D3" V 10738 5583 50  0000 R CNN
F 1 "LED" V 10647 5583 50  0000 R CNN
F 2 "LED_SMD:LED_2010_5025Metric_Castellated" H 10700 5700 50  0001 C CNN
F 3 "~" H 10700 5700 50  0001 C CNN
	1    10700 5700
	0    -1   -1   0   
$EndComp
$Comp
L Connector:Conn_01x02_Male J4
U 1 1 5D015A24
P 7700 5600
AR Path="/5D015A24" Ref="J4"  Part="1" 
AR Path="/5D015685/5D015A24" Ref="J?"  Part="1" 
F 0 "J4" H 7806 5778 50  0000 C CNN
F 1 "AC_IN" H 7806 5687 50  0000 C CNN
F 2 "TerminalBlock:TerminalBlock_Altech_AK300-2_P5.00mm" H 7700 5600 50  0001 C CNN
F 3 "~" H 7700 5600 50  0001 C CNN
	1    7700 5600
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR014
U 1 1 5D015A2B
P 10200 5350
AR Path="/5D015A2B" Ref="#PWR014"  Part="1" 
AR Path="/5D015685/5D015A2B" Ref="#PWR?"  Part="1" 
F 0 "#PWR014" H 10200 5200 50  0001 C CNN
F 1 "+5V" H 10215 5523 50  0000 C CNN
F 2 "" H 10200 5350 50  0001 C CNN
F 3 "" H 10200 5350 50  0001 C CNN
	1    10200 5350
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR019
U 1 1 5D015A31
P 9300 5950
AR Path="/5D015A31" Ref="#PWR019"  Part="1" 
AR Path="/5D015685/5D015A31" Ref="#PWR?"  Part="1" 
F 0 "#PWR019" H 9300 5700 50  0001 C CNN
F 1 "GND" H 9305 5777 50  0000 C CNN
F 2 "" H 9300 5950 50  0001 C CNN
F 3 "" H 9300 5950 50  0001 C CNN
	1    9300 5950
	1    0    0    -1  
$EndComp
$Comp
L power:+12V #PWR015
U 1 1 5D015A37
P 9300 5400
AR Path="/5D015A37" Ref="#PWR015"  Part="1" 
AR Path="/5D015685/5D015A37" Ref="#PWR?"  Part="1" 
F 0 "#PWR015" H 9300 5250 50  0001 C CNN
F 1 "+12V" H 9315 5573 50  0000 C CNN
F 2 "" H 9300 5400 50  0001 C CNN
F 3 "" H 9300 5400 50  0001 C CNN
	1    9300 5400
	1    0    0    -1  
$EndComp
$Comp
L Device:CP_Small C1
U 1 1 5D015A3D
P 9300 5700
AR Path="/5D015A3D" Ref="C1"  Part="1" 
AR Path="/5D015685/5D015A3D" Ref="C?"  Part="1" 
F 0 "C1" H 9388 5746 50  0000 L CNN
F 1 "100uF 35V" H 9300 5650 50  0000 L CNN
F 2 "Capacitor_SMD:CP_Elec_5x5.4" H 9300 5700 50  0001 C CNN
F 3 "~" H 9300 5700 50  0001 C CNN
	1    9300 5700
	1    0    0    -1  
$EndComp
$Comp
L Device:CP_Small C2
U 1 1 5D015A44
P 10200 5700
AR Path="/5D015A44" Ref="C2"  Part="1" 
AR Path="/5D015685/5D015A44" Ref="C?"  Part="1" 
F 0 "C2" H 10288 5746 50  0000 L CNN
F 1 "10uF 35V" H 10288 5655 50  0000 L CNN
F 2 "Capacitor_SMD:CP_Elec_5x5.4" H 10200 5700 50  0001 C CNN
F 3 "~" H 10200 5700 50  0001 C CNN
	1    10200 5700
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR020
U 1 1 5D015A4B
P 9750 5950
AR Path="/5D015A4B" Ref="#PWR020"  Part="1" 
AR Path="/5D015685/5D015A4B" Ref="#PWR?"  Part="1" 
F 0 "#PWR020" H 9750 5700 50  0001 C CNN
F 1 "GND" H 9755 5777 50  0000 C CNN
F 2 "" H 9750 5950 50  0001 C CNN
F 3 "" H 9750 5950 50  0001 C CNN
	1    9750 5950
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR021
U 1 1 5D015A51
P 10200 5950
AR Path="/5D015A51" Ref="#PWR021"  Part="1" 
AR Path="/5D015685/5D015A51" Ref="#PWR?"  Part="1" 
F 0 "#PWR021" H 10200 5700 50  0001 C CNN
F 1 "GND" H 10205 5777 50  0000 C CNN
F 2 "" H 10200 5950 50  0001 C CNN
F 3 "" H 10200 5950 50  0001 C CNN
	1    10200 5950
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR022
U 1 1 5D015A57
P 10700 5950
AR Path="/5D015A57" Ref="#PWR022"  Part="1" 
AR Path="/5D015685/5D015A57" Ref="#PWR?"  Part="1" 
F 0 "#PWR022" H 10700 5700 50  0001 C CNN
F 1 "GND" H 10705 5777 50  0000 C CNN
F 2 "" H 10700 5950 50  0001 C CNN
F 3 "" H 10700 5950 50  0001 C CNN
	1    10700 5950
	1    0    0    -1  
$EndComp
Wire Wire Line
	9300 5800 9300 5950
Wire Wire Line
	9750 5800 9750 5950
Wire Wire Line
	10200 5800 10200 5950
Wire Wire Line
	10700 5850 10700 5950
Wire Wire Line
	10050 5500 10200 5500
Wire Wire Line
	10200 5600 10200 5500
Connection ~ 10200 5500
Wire Wire Line
	10200 5500 10300 5500
Wire Wire Line
	10200 5500 10200 5350
Wire Wire Line
	10600 5500 10700 5500
Wire Wire Line
	10700 5500 10700 5550
Wire Wire Line
	9300 5400 9300 5500
Wire Wire Line
	9450 5500 9300 5500
Connection ~ 9300 5500
Wire Wire Line
	9300 5500 9300 5600
$Comp
L power:+5V #PWR07
U 1 1 5D015A8B
P 8800 1850
AR Path="/5D015A8B" Ref="#PWR07"  Part="1" 
AR Path="/5D015685/5D015A8B" Ref="#PWR?"  Part="1" 
F 0 "#PWR07" H 8800 1700 50  0001 C CNN
F 1 "+5V" H 8815 2023 50  0000 C CNN
F 2 "" H 8800 1850 50  0001 C CNN
F 3 "" H 8800 1850 50  0001 C CNN
	1    8800 1850
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR011
U 1 1 5D015AB0
P 8800 3300
AR Path="/5D015AB0" Ref="#PWR011"  Part="1" 
AR Path="/5D015685/5D015AB0" Ref="#PWR?"  Part="1" 
F 0 "#PWR011" H 8800 3050 50  0001 C CNN
F 1 "GND" H 8805 3127 50  0000 C CNN
F 2 "" H 8800 3300 50  0001 C CNN
F 3 "" H 8800 3300 50  0001 C CNN
	1    8800 3300
	1    0    0    -1  
$EndComp
Text GLabel 9400 2800 2    50   Input ~ 0
Reset
Text GLabel 9400 2300 2    50   Input ~ 0
MOSI
Text GLabel 9400 2400 2    50   Input ~ 0
MISO
Text GLabel 9400 2500 2    50   Input ~ 0
SCK
Text GLabel 10350 2800 0    50   Input ~ 0
MOSI
Text GLabel 10350 2900 0    50   Input ~ 0
MISO
Text GLabel 10350 3000 0    50   Input ~ 0
SCK
Wire Wire Line
	10350 2800 10600 2800
Wire Wire Line
	10350 2900 10600 2900
Wire Wire Line
	10350 3000 10600 3000
$Comp
L power:+5V #PWR012
U 1 1 5D015ADC
P 10400 3550
AR Path="/5D015ADC" Ref="#PWR012"  Part="1" 
AR Path="/5D015685/5D015ADC" Ref="#PWR?"  Part="1" 
F 0 "#PWR012" H 10400 3400 50  0001 C CNN
F 1 "+5V" H 10415 3723 50  0000 C CNN
F 2 "" H 10400 3550 50  0001 C CNN
F 3 "" H 10400 3550 50  0001 C CNN
	1    10400 3550
	1    0    0    -1  
$EndComp
$Comp
L Device:R_US R7
U 1 1 5D015AE2
P 10400 3750
AR Path="/5D015AE2" Ref="R7"  Part="1" 
AR Path="/5D015685/5D015AE2" Ref="R?"  Part="1" 
F 0 "R7" H 10468 3796 50  0000 L CNN
F 1 "10K" H 10468 3705 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" V 10440 3740 50  0001 C CNN
F 3 "~" H 10400 3750 50  0001 C CNN
	1    10400 3750
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR013
U 1 1 5D015AE9
P 10400 4550
AR Path="/5D015AE9" Ref="#PWR013"  Part="1" 
AR Path="/5D015685/5D015AE9" Ref="#PWR?"  Part="1" 
F 0 "#PWR013" H 10400 4300 50  0001 C CNN
F 1 "GND" H 10405 4377 50  0000 C CNN
F 2 "" H 10400 4550 50  0001 C CNN
F 3 "" H 10400 4550 50  0001 C CNN
	1    10400 4550
	1    0    0    -1  
$EndComp
Wire Wire Line
	10400 3550 10400 3600
Wire Wire Line
	10400 3900 10400 4000
Wire Wire Line
	10400 4450 10400 4550
Wire Wire Line
	10400 4000 10450 4000
Connection ~ 10400 4000
Wire Wire Line
	10400 4000 10400 4050
Text GLabel 10450 4000 2    50   Input ~ 0
Reset
$Comp
L Connector_Generic:Conn_02x03_Odd_Even J3
U 1 1 5D015B0A
P 10500 2100
AR Path="/5D015B0A" Ref="J3"  Part="1" 
AR Path="/5D015685/5D015B0A" Ref="J?"  Part="1" 
F 0 "J3" H 10550 2417 50  0000 C CNN
F 1 "ICSP" H 10550 2326 50  0000 C CNN
F 2 "Connector_PinHeader_2.00mm:PinHeader_2x03_P2.00mm_Vertical_SMD" H 10500 2100 50  0001 C CNN
F 3 "~" H 10500 2100 50  0001 C CNN
	1    10500 2100
	1    0    0    -1  
$EndComp
Text GLabel 10800 2100 2    50   Input ~ 0
MOSI
Text GLabel 10300 2000 0    50   Input ~ 0
MISO
Text GLabel 10300 2100 0    50   Input ~ 0
SCK
Text GLabel 10300 2200 0    50   Input ~ 0
Reset
$Comp
L power:GND #PWR09
U 1 1 5D015B15
P 10850 2300
AR Path="/5D015B15" Ref="#PWR09"  Part="1" 
AR Path="/5D015685/5D015B15" Ref="#PWR?"  Part="1" 
F 0 "#PWR09" H 10850 2050 50  0001 C CNN
F 1 "GND" H 10855 2127 50  0000 C CNN
F 2 "" H 10850 2300 50  0001 C CNN
F 3 "" H 10850 2300 50  0001 C CNN
	1    10850 2300
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR08
U 1 1 5D015B1B
P 10850 1900
AR Path="/5D015B1B" Ref="#PWR08"  Part="1" 
AR Path="/5D015685/5D015B1B" Ref="#PWR?"  Part="1" 
F 0 "#PWR08" H 10850 1750 50  0001 C CNN
F 1 "+5V" H 10865 2073 50  0000 C CNN
F 2 "" H 10850 1900 50  0001 C CNN
F 3 "" H 10850 1900 50  0001 C CNN
	1    10850 1900
	1    0    0    -1  
$EndComp
Wire Wire Line
	10800 2200 10850 2200
Wire Wire Line
	10850 2200 10850 2300
Wire Wire Line
	10800 2000 10850 2000
Wire Wire Line
	10850 2000 10850 1900
Wire Notes Line
	7550 700  11150 700 
Wire Notes Line
	11150 700  11150 6250
Wire Notes Line
	11150 6250 7550 6250
Wire Notes Line
	7550 6250 7550 700 
Text Notes 7600 850  0    50   ~ 10
MCU
Wire Wire Line
	8800 2000 8800 1850
Wire Wire Line
	8800 3300 8800 3200
Text GLabel 10600 2800 2    50   Input ~ 10
PB0
Text GLabel 10600 2900 2    50   Input ~ 10
PB1
Text GLabel 10600 3000 2    50   Input ~ 10
PB2
Wire Wire Line
	9400 2600 9850 2600
Wire Wire Line
	9400 2700 9600 2700
Text Label 9850 2600 2    50   ~ 0
Relay
$Comp
L Relay:SANYOU_SRD_Form_C K2
U 1 1 5CF5533A
P 5450 2350
F 0 "K2" V 4883 2350 50  0000 C CNN
F 1 "SANYOU_SRD_Form_C" V 4974 2350 50  0000 C CNN
F 2 "Relay_THT:Relay_SPDT_SANYOU_SRD_Series_Form_C" H 5900 2300 50  0001 L CNN
F 3 "http://www.sanyourelay.ca/public/products/pdf/SRD.pdf" H 5450 2350 50  0001 C CNN
	1    5450 2350
	0    1    1    0   
$EndComp
Text Label 6350 4100 0    50   ~ 0
FP_Input
Wire Wire Line
	5200 4400 5200 4600
Wire Wire Line
	6000 4400 6000 4600
Wire Wire Line
	4850 4600 4850 4500
$Comp
L power:GND #PWR02
U 1 1 5CF58449
P 4850 4600
F 0 "#PWR02" H 4850 4350 50  0001 C CNN
F 1 "GND" H 4855 4427 50  0000 C CNN
F 2 "" H 4850 4600 50  0001 C CNN
F 3 "" H 4850 4600 50  0001 C CNN
	1    4850 4600
	1    0    0    -1  
$EndComp
Wire Wire Line
	5200 4400 5300 4400
Wire Wire Line
	4850 4200 5000 4200
Wire Wire Line
	4850 4300 4850 4200
Connection ~ 4850 4200
Wire Wire Line
	4700 4200 4850 4200
Wire Wire Line
	5200 4200 5300 4200
$Comp
L power:GND #PWR03
U 1 1 5CF548B1
P 5200 4600
F 0 "#PWR03" H 5200 4350 50  0001 C CNN
F 1 "GND" H 5205 4427 50  0000 C CNN
F 2 "" H 5200 4600 50  0001 C CNN
F 3 "" H 5200 4600 50  0001 C CNN
	1    5200 4600
	1    0    0    -1  
$EndComp
Wire Wire Line
	6000 4100 6000 4050
Wire Wire Line
	6000 4100 6350 4100
Wire Wire Line
	6000 3700 6000 3850
$Comp
L power:+5V #PWR04
U 1 1 5CF537D3
P 6000 3700
F 0 "#PWR04" H 6000 3550 50  0001 C CNN
F 1 "+5V" H 6015 3873 50  0000 C CNN
F 2 "" H 6000 3700 50  0001 C CNN
F 3 "" H 6000 3700 50  0001 C CNN
	1    6000 3700
	1    0    0    -1  
$EndComp
Connection ~ 6000 4100
Wire Wire Line
	6000 4200 6000 4100
Wire Wire Line
	5900 4200 6000 4200
Wire Wire Line
	5900 4400 6000 4400
$Comp
L power:GND #PWR05
U 1 1 5CF5261A
P 6000 4600
F 0 "#PWR05" H 6000 4350 50  0001 C CNN
F 1 "GND" H 6005 4427 50  0000 C CNN
F 2 "" H 6000 4600 50  0001 C CNN
F 3 "" H 6000 4600 50  0001 C CNN
	1    6000 4600
	1    0    0    -1  
$EndComp
$Comp
L Device:R_Small_US R5
U 1 1 5CF52362
P 6000 3950
F 0 "R5" H 5932 3904 50  0000 R CNN
F 1 "4.7K" H 5932 3995 50  0000 R CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 6000 3950 50  0001 C CNN
F 3 "~" H 6000 3950 50  0001 C CNN
	1    6000 3950
	-1   0    0    1   
$EndComp
$Comp
L Device:R_Small_US R3
U 1 1 5CF51EBE
P 4850 4400
F 0 "R3" H 4782 4354 50  0000 R CNN
F 1 "1K" H 4782 4445 50  0000 R CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 4850 4400 50  0001 C CNN
F 3 "~" H 4850 4400 50  0001 C CNN
	1    4850 4400
	-1   0    0    1   
$EndComp
$Comp
L Device:R_Small_US R2
U 1 1 5CF51BC8
P 4600 4200
F 0 "R2" V 4395 4200 50  0000 C CNN
F 1 "2K" V 4486 4200 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 4600 4200 50  0001 C CNN
F 3 "~" H 4600 4200 50  0001 C CNN
	1    4600 4200
	0    1    1    0   
$EndComp
$Comp
L Device:R_Small_US R4
U 1 1 5CF515CC
P 5100 4200
F 0 "R4" V 4895 4200 50  0000 C CNN
F 1 "10K" V 4986 4200 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 5100 4200 50  0001 C CNN
F 3 "~" H 5100 4200 50  0001 C CNN
	1    5100 4200
	0    1    1    0   
$EndComp
$Comp
L Isolator:PC817 U1
U 1 1 5CF5027C
P 5600 4300
F 0 "U1" H 5600 4625 50  0000 C CNN
F 1 "PC817" H 5600 4534 50  0000 C CNN
F 2 "Package_DIP:DIP-4_W7.62mm" H 5400 4100 50  0001 L CIN
F 3 "http://www.soselectronic.cz/a_info/resource/d/pc817.pdf" H 5600 4300 50  0001 L CNN
	1    5600 4300
	1    0    0    -1  
$EndComp
Wire Wire Line
	4950 2850 6100 2850
Wire Wire Line
	4950 2550 4950 2850
Wire Wire Line
	4350 2800 4350 2600
$Comp
L power:GND #PWR01
U 1 1 5CF493CB
P 4350 2800
F 0 "#PWR01" H 4350 2550 50  0001 C CNN
F 1 "GND" H 4355 2627 50  0000 C CNN
F 2 "" H 4350 2800 50  0001 C CNN
F 3 "" H 4350 2800 50  0001 C CNN
	1    4350 2800
	1    0    0    -1  
$EndComp
Text Label 6150 5400 0    50   ~ 0
Rela_y
Text Label 9600 2700 0    50   ~ 0
Rela_y
Wire Wire Line
	4150 4200 4500 4200
Wire Wire Line
	5750 2450 6400 2450
Wire Wire Line
	5750 2650 6400 2650
Wire Wire Line
	6100 2550 6100 2850
Text Label 7950 3950 0    50   ~ 0
FP_Input
Text GLabel 7950 3950 0    50   Input ~ 10
PB0
$Comp
L Elevator_FirePanel-Board-rescue:ATtiny13-20SSU-MCU_Microchip_ATtiny U2
U 1 1 5D21BC6E
P 8800 2600
F 0 "U2" H 8271 2646 50  0000 R CNN
F 1 "ATtiny13-20SSU" H 8271 2555 50  0000 R CNN
F 2 "Package_SO:SOIC-8_3.9x4.9mm_P1.27mm" H 8800 2600 50  0001 C CIN
F 3 "http://ww1.microchip.com/downloads/en/DeviceDoc/doc2535.pdf" H 8800 2600 50  0001 C CNN
	1    8800 2600
	1    0    0    -1  
$EndComp
$Comp
L Regulator_Linear:AMS1117-5.0 U3
U 1 1 5D21DAEE
P 9750 5500
F 0 "U3" H 9750 5742 50  0000 C CNN
F 1 "AMS1117-5.0" H 9750 5651 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-223-3_TabPin2" H 9750 5700 50  0001 C CNN
F 3 "http://www.advanced-monolithic.com/pdf/ds1117.pdf" H 9850 5250 50  0001 C CNN
	1    9750 5500
	1    0    0    -1  
$EndComp
$Comp
L Device:D_Bridge_-AA+ D4
U 1 1 5D1E322A
P 8500 5650
F 0 "D4" H 8156 5621 50  0000 R CNN
F 1 "D_Bridge_-AA+" H 8156 5695 28  0000 R CNN
F 2 "Diode_THT:Diode_Bridge_19.0x3.5x10.0mm_P5.0mm" H 8500 5650 50  0001 C CNN
F 3 "~" H 8500 5650 50  0001 C CNN
	1    8500 5650
	-1   0    0    1   
$EndComp
Wire Wire Line
	7900 5600 7900 5300
Wire Wire Line
	7900 5300 8500 5300
Wire Wire Line
	8500 5300 8500 5350
Wire Wire Line
	7900 5700 7900 6000
Wire Wire Line
	7900 6000 8500 6000
Wire Wire Line
	8500 6000 8500 5950
Wire Wire Line
	8200 5650 8150 5650
Wire Wire Line
	8150 5650 8150 5200
Wire Wire Line
	8150 5200 9000 5200
Wire Wire Line
	9000 5200 9000 5500
Wire Wire Line
	9000 5500 9300 5500
Wire Wire Line
	8800 5650 8850 5650
Wire Wire Line
	8850 5650 8850 5850
$Comp
L power:GND #PWR0101
U 1 1 5D1EE841
P 8850 5850
AR Path="/5D1EE841" Ref="#PWR0101"  Part="1" 
AR Path="/5D015685/5D1EE841" Ref="#PWR?"  Part="1" 
F 0 "#PWR0101" H 8850 5600 50  0001 C CNN
F 1 "GND" H 8855 5677 50  0000 C CNN
F 2 "" H 8850 5850 50  0001 C CNN
F 3 "" H 8850 5850 50  0001 C CNN
	1    8850 5850
	1    0    0    -1  
$EndComp
$EndSCHEMATC
